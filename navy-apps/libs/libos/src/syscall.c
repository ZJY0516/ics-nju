...
{
    _syscall_(SYS_gettimeofday, (intptr_t)tv, (intptr_t)NULL, 0);
    return 0;
}

int _execve(const char *fname, char *const argv[], char *const envp[])
{
    ptr_brk = (intptr_t)&end; // i don't know where to handle heap;
    return _syscall_(SYS_execve, (intptr_t)fname, (intptr_t)argv,
                     (intptr_t)envp);
}
